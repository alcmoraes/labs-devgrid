...
class Utils {
    ...
    scrollToElement( destination: ?HTMLElement = null, duration?: number = 200, easing?: string = 'linear', threshold?: number = 0, callback?: Function ): void{
      ...
      if ( document.body && document.documentElement ){
        ...
        const documentHeight = Math.max( document.body.scrollHeight, document.body.offsetHeight, documentElementClientHeight, document.documentElement.scrollHeight, document.documentElement.offsetHeight );
        const windowHeight = window.innerHeight || documentElementClientHeight || document.getElementsByTagName( 'body' )[ 0 ].clientHeight;
        const destinationOffset = typeof destination === 'number' ? destination : destination.offsetTop;
        const destinationOffsetToScroll = Math.round( documentHeight - destinationOffset < windowHeight ? documentHeight - windowHeight : destinationOffset ) + threshold;
